...
- `components/dashboard/Skeleton.tsx` 의 `SkeletonBlock`
...
components/
  ...
  features/
    ...
    random/    RandomDrawer RandomModeTabs DrawSlot DestinationCard
      dart/    DartBoard KoreaMapLayer DartResultCard useDartThrow.ts
...
`components/dashboard/DartGame.tsx` 333줄 — 컨벤션 200줄 초과.

- `features/random/dart/useDartThrow.ts` — 상태머신(`Mode`/`Phase`/`Outcome`),
  포인터 핸들러, rAF 비행 애니메이션, 히트 테스트, 정리(cleanup)
...
`setPointerCapture` try/catch 등 기존 주석에 남은 "왜"는 그대로 옮긴다.
